...
def run(args):
    ...
    for ep in range(args.irn_num_epoches):
        ...
        for pack in train_data_loader:
            ...
            # print(iter)
            img = pack['img'].cuda()
            bg_pos_label = pack['aff_bg_pos_label'].cuda()
            fg_pos_label = pack['aff_fg_pos_label'].cuda()
            neg_label = pack['aff_neg_label'].cuda()
            ...
            timer.reset_stage()
        torch.save(model.state_dict(), args.irn_weights_name + str(ep))
    #
    ...
    # print('done.')
    # #
    # torch.save(model.state_dict(), args.irn_weights_name + str(args.irn_num_epoches) + "mean")
