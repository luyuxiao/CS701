...
CAT_LIST = []
with open('/home/yuxiao/CS701/CONTA/pseudo_mask/FOOD103/classes.txt', 'r') as f:
    data = f.readlines()
    for line in data:
...
cls_labels_dict_train = np.load('/home/yuxiao/CS701/CONTA/pseudo_mask/voc12/food103_cls_labels_train.npy', allow_pickle=True).item()

cls_labels_dict_val = None
if os.path.exists('/home/yuxiao/CS701/CONTA/pseudo_mask/voc12/food103_cls_labels_val.npy'):
    cls_labels_dict_val = np.load('/home/yuxiao/CS701/CONTA/pseudo_mask/voc12/food103_cls_labels_val.npy', allow_pickle=True).item()
...
def load_image_label_list_from_npy(img_name_list, train=True):
    if train:
        return np.array([cls_labels_dict_train[img_name] for img_name in img_name_list])
    else:
        return np.array([cls_labels_dict_val[img_name] for img_name in img_name_list])

def get_img_path(img_name, voc12_root, sub_dir=None):
    # if not isinstance(img_name, str):
    #     img_name = decode_int_filename(img_name)

    return os.path.join(voc12_root,sub_dir, img_name + '.jpg')
...
class VOC12ImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        img = np.asarray(imageio.imread(get_img_path(name, self.voc12_root, self.food_sub_dir)))
...
class VOC12ClassificationDataset(VOC12ImageDataset):
    ...
    def __init__(self, img_name_list_path, voc12_root, food_sub_dir='',
        ...
        super().__init__(img_name_list_path, voc12_root, food_sub_dir,
                 resize_long, rescale, img_normal, hor_flip,
                 crop_size, crop_method)

        if food_sub_dir=='train':
            self.label_list = load_image_label_list_from_npy(self.img_name_list)
        else:
            # if cls_labels_dict_val is None:
            self.label_list = np.zeros((len(self.img_name_list),103))
            # else:
            #     self.label_list = load_image_label_list_from_npy(self.img_name_list)
...
class VOC12ClassificationDatasetMSF(VOC12ClassificationDataset):
    ...
    def __init__(self, img_name_list_path, voc12_root,sub_dir,
                 img_normal=TorchvisionNormalize(),
                 scales=(1.0,)):
        self.scales = scales
        self.sub_dir = sub_dir

        super().__init__(img_name_list_path, voc12_root,sub_dir, img_normal=img_normal)
        self.scales = scales

        if sub_dir=='val':
            cls_labels_dict_val = np.load('voc12/food103_cls_labels_val.npy', allow_pickle=True).item()
            self.label_list = np.array([cls_labels_dict_val[img_name] for img_name in self.img_name_list])

    def __getitem__(self, idx):
        name = self.img_name_list[idx]
        # name_str = decode_int_filename(name)

        img = imageio.imread(get_img_path(name, self.voc12_root, self.sub_dir))
...
class VOC12SegmentationDataset(Dataset):
    ...
    def __init__(self, img_name_list_path, label_dir, crop_size, voc12_root,
                 ...
                 crop_method = 'random', food_sub_dir=None):
    ...
    def __getitem__(self, idx):
        ...
        # print(get_img_path(name, self.voc12_root, self.food_sub_dir))
        img = imageio.imread(get_img_path(name, self.voc12_root, self.food_sub_dir))

        label = imageio.imread(os.path.join(self.label_dir, name_str + '.png'))

        img = np.asarray(img)
        # downsample_size = max(img.shape[0] - 1, img.shape[1] - 1) // 512 + 1
        # if downsample_size > 1:
        #     img = img[::downsample_size, ::downsample_size, :]
        #     label = label[::downsample_size, ::downsample_size]
...
class VOC12AffinityDataset(VOC12SegmentationDataset):
    def __init__(self, img_name_list_path, label_dir, crop_size, voc12_root,
                 ...
                 rescale=None, img_normal=TorchvisionNormalize(), hor_flip=False, crop_method=None, food_sub_dir=None):
        super().__init__(img_name_list_path, label_dir, crop_size, voc12_root, rescale, img_normal, hor_flip, crop_method=crop_method, food_sub_dir=food_sub_dir)
...
class VOC12RIBDataset(VOC12ImageDataset):
    ...
    def find_disjoint_class_images(self):
        ...
        img_index = list(self.img_name_list).index(self.image_id)
    ...
    def find_joint_class_images(self):
        ...
        img_index = list(self.total_list).index(self.image_id)
